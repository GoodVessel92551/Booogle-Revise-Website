...
const spellCheck = (word) => {
  ...
  for (let i = 0; i < dictionary.length; i++) {
    const distance = levenshteinDistance(word.replaceAll(" ", ""), dictionary[i]);
  ...
  return {
    word: bestMatch,
    confidence: 1 - bestDistance / Math.max(word.replaceAll(" ", "").length, bestMatch.length),
  };
};
...
const autocomplete = (sortedWords) => {
  const complete = (input, current, next, button) => {
    ...
    /*if (word_list.length > 1) {
      ...
      var spell = spellCheck(word_before.toLowerCase(),current);
...
if(document.getElementById("search")){
      ...
      var current = document.getElementById("search_autocomplete_current");
      var next = document.getElementById("search_autocomplete_next");
      var button = document.getElementById("search_autocomplete_button");
